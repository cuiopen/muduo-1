...
namespace {
  ...
  int createEventfd() {
    ...
    if (evtfd < 0) {
      SYSLOG(ERROR) << "Failed in eventfd";
      abort();
    }
    return evtfd;
  }
    ...
    currentActiveChannel_(NULL)
{
  if (t_loopInThisThread) {
    LOG(FATAL) << "Another EventLoop " << t_loopInThisThread << " exists in this thread " << threadId_;
  }
  else {
...
//    }
    ...
    perf_counter::last_loop_time = Timestamp::now().microSecondsSinceEpoch()
        - pollReturnTime_.microSecondsSinceEpoch();
  }
