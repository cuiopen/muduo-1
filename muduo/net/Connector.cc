...
Connector::Connector(EventLoop* loop, const InetAddress& serverAddr) :
    loop_(loop), serverAddr_(serverAddr), connect_(false), state_(kDisconnected), retryDelayMs_(
        kInitRetryDelayMs) {
  DLOG(INFO) << "ctor[" << this << "]";
}
...
void Connector::handleWrite() {
  DLOG(INFO) << "Connector::handleWrite " << state_;
  ...
  if (state_ == kConnecting) {
    ...
    if (err) {
      LOG(WARNING) << "Connector::handleWrite - SO_ERROR = " << err << " " << std::strerror(err);
      retry(sockfd);
    }
...
void Connector::handleError() {
  ...
  LOG(WARNING) << err << " " << std::strerror(err);
